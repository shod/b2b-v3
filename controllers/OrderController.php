...
class OrderController extends Controller
{
    ...
    private function history($page=0) {
        ...
        if(!empty($history)) {
            foreach($history as $ar) {
                ...
                if($ar['status'] == '4') {$status_order = "<span style='color:rgba(47,0,185,0.91)'>Возврат срдств</span>";}
    ...
    private function getHistoryRow($order_id){
        ...
        if($order['status'] == '4') {$status_order = "<span style='color:rgba(47,0,185,0.91)'>Возврат срдств</span>";}
